...
describe Atc::Aws::FixityCheck do
  ...
  describe '#get_fixity_checksum_object_size' do
    ...
    it 'calls #process_aws_fixity_websocket_channel_stream_response' do
      expect(aws_fixity_check).to receive(:process_aws_fixity_websocket_channel_stream_response)
      aws_fixity_check.fixity_checksum_object_size
    end
    ...
    context 'with an AWS response without errors ' do
      it 'returns the object checksum and object size, and nil for the aws error message' do
        allow(aws_fixity_check).to receive(:aws_fixity_websocket_channel_stream) { aws_json_response }
    ...
    context 'with an AWS response with errors ' do
      it 'returns nil for the object checksum and object size, and the aws error message' do
        allow(aws_fixity_check).to receive(:aws_fixity_websocket_channel_stream) { aws_error_json_response }
        result = aws_fixity_check.fixity_checksum_object_size
