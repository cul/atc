...
describe PendingTransfer do
  ...
  describe 'validations' do
    it 'fails to save when a positive size object has a zero-byte checksum value' do
      pending_transfer.transfer_checksum_value = pending_transfer.transfer_checksum_algorithm.empty_value
  ...
  context 'with a no-content checksum' do
    let(:pending_transfer_with_zero_byte_size_and_empty_value_checksum) do
    ...
    let(:pending_transfer_with_positive_byte_size_and_empty_value_checksum) do
    ...
    it 'saves without error the object size is zero' do
      expect(pending_transfer_with_zero_byte_size_and_empty_value_checksum.save).to eq(true)
    end

    it 'fails to save when object size is a positive number' do
      expect(pending_transfer_with_positive_byte_size_and_empty_value_checksum.save).to eq(false)
      expect(
        pending_transfer_with_positive_byte_size_and_empty_value_checksum.errors
